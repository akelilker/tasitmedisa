...
(function() {
  ...
  var monthlyTodoWaSaveInflight = Object.create(null);
  var MONTHLY_TODO_WA_AUDIT_PAGE_SIZE = 50;
  var monthlyTodoWaAuditState = {
    search: '',
    branchId: 'all',
    typeCode: 'all',
    timeFilter: 'all',
    limit: MONTHLY_TODO_WA_AUDIT_PAGE_SIZE
  };

  function deepCloneMonthlyTodoWaLogEntry(entry) {
  ...
  function applyMonthlyTodoWhatsAppOpenedUiState(anchorEl) {
    if (!anchorEl) return;
  ...
  function dispatchMonthlyTodoWhatsAppLogsChanged(reminderKey, action, updatedAt) {
    try {
      window.dispatchEvent(new CustomEvent('medisa:monthly-todo-whatsapp-logs-changed', {
        detail: {
  ...
  function recordMonthlyTodoWhatsAppOpened(reminderKey, anchorEl) {
    ...
    if (monthlyTodoWaSaveInflight[reminderKey]) {
      alert('Kayıt işlemi devam ediyor');
      return;
    }
    var logs = ensureMonthlyTodoWhatsAppLogs();
    var hadPrevious = Object.prototype.hasOwnProperty.call(logs, reminderKey);
    var previous = hadPrevious ? deepCloneMonthlyTodoWaLogEntry(logs[reminderKey]) : null;
    ...
    if (previous && typeof previous === 'object') {
      nextEntry = {
        vehicleId: String(previous.vehicleId || vehicleId || ''),
        plate: plate || String(previous.plate || ''),
        type: typeCode || String(previous.type || ''),
        field: field || String(previous.field || ''),
        date: date || String(previous.date || ''),
        firstOpenedAt: String(previous.firstOpenedAt || nowIso),
        lastOpenedAt: nowIso,
        openedCount: (Number(previous.openedCount) || 0) + 1,
        openedBy: String(openedBy || previous.openedBy || '')
      };
    } else {
      nextEntry = {
        vehicleId: vehicleId,
    ...
    logs[reminderKey] = nextEntry;
    monthlyTodoWaSaveInflight[reminderKey] = true;

    function rollbackLog() {
      if (hadPrevious && previous) logs[reminderKey] = previous;
      else delete logs[reminderKey];
    }

    function finishInflight() {
      delete monthlyTodoWaSaveInflight[reminderKey];
    }

    function failPersist() {
      rollbackLog();
      finishInflight();
      alert('WhatsApp bağlantısı açıldı ancak bildirim geçmişi sunucuya kaydedilemedi.');
    }
    ...
    window.saveDataToServer().then(function(ok) {
      ...
      applyMonthlyTodoWhatsAppOpenedUiState(anchorEl);
      finishInflight();
      dispatchMonthlyTodoWhatsAppLogsChanged(reminderKey, 'upsert', nextEntry.lastOpenedAt);
      if (document.getElementById('monthly-todo-whatsapp-audit-modal') &&
          document.getElementById('monthly-todo-whatsapp-audit-modal').classList.contains('active')) {
        renderMonthlyTodoWhatsAppAuditModalContent();
      }
    }).catch(function(err) {
      if (typeof window.__medisaLogError === 'function') {
        window.__medisaLogError('monthlyTodoWhatsAppLogs kaydı başarısız', err);
      } else {
  ...
  function getMonthlyTodoWaTypeLabel(typeCode, field) {
    ...
    if (fieldStr) {
      try {
        var kind = resolveMonthlyTodoTaskKind({ type: '', field: fieldStr });
        if (kind) return kind;
      } catch (_e) {}
      return fieldStr;
    }
  ...
  function normalizeMonthlyTodoWhatsAppAuditEntries(rawLogs, vehicleMap, branchNameMap) {
    ...
    Object.keys(rawLogs).forEach(function(key) {
      ...
      if (!entry || typeof entry !== 'object' || Array.isArray(entry)) {
        console.warn('[Medisa] monthlyTodoWhatsAppLogs bozuk kayıt atlandı');
        return;
      }
      var vehicleId = String(entry.vehicleId || '').trim();
      var vehicle = vehicleId && vehicleMap ? vehicleMap[vehicleId] : null;
      var plate = String((vehicle && vehicle.plate) || entry.plate || '').trim() || '—';
      var branchId = vehicle && vehicle.branchId != null ? String(vehicle.branchId) : '';
      ...
      var typeLabel = getMonthlyTodoWaTypeLabel(typeCode, field);
      var countRaw = Number(entry.openedCount);
      var openedCount = Number.isFinite(countRaw) && countRaw > 0 ? Math.floor(countRaw) : 0;
      var firstMs = parseMonthlyTodoWaTimeMs(entry.firstOpenedAt);
      var lastMs = parseMonthlyTodoWaTimeMs(entry.lastOpenedAt);
      out.push({
        reminderKey: String(key),
        vehicleId: vehicleId,
        plate: plate,
        brandModel: vehicle ? String(vehicle.brandModel || '').trim() : '',
        branchId: branchId,
        ...
        firstOpenedAt: String(entry.firstOpenedAt || ''),
        lastOpenedAt: String(entry.lastOpenedAt || ''),
        firstOpenedMs: firstMs,
        lastOpenedMs: lastMs,
    ...
    out.sort(function(a, b) {
      ...
      var plateCmp = String(a.plate).localeCompare(String(b.plate), 'tr');
      if (plateCmp) return plateCmp;
      return String(a.reminderKey).localeCompare(String(b.reminderKey));
    });
    return out;
  }
  ...
  function filterMonthlyTodoWhatsAppAuditEntries(entries, state) {
    ...
    var st = state || monthlyTodoWaAuditState;
    var q = String(st.search || '').trim().toLocaleLowerCase('tr-TR');
    var branchId = String(st.branchId || 'all');
    var typeCode = String(st.typeCode || 'all');
    var timeFilter = String(st.timeFilter || 'all');
    var now = Date.now();
    ...
    return list.filter(function(row) {
      ...
      if (typeCode !== 'all') {
        ...
        if (rowType !== String(typeCode).toLowerCase() && String(row.typeLabel || '') !== typeCode) return false;
      }
      if (timeFilter !== 'all') {
        var ms = row.lastOpenedMs || 0;
        if (!ms) return false;
        if (timeFilter === '7d' && ms < now - 7 * 86400000) return false;
        if (timeFilter === '30d' && ms < now - 30 * 86400000) return false;
        if (timeFilter === 'month' && ms < monthStartMs) return false;
      }
      if (!q) return true;
      var hay = [
        row.plate,
  ...
  function summarizeMonthlyTodoWhatsAppAuditEntries(entries) {
    ...
    return {
      ...
      lastOpenedDisplay: lastIso ? formatDateForDisplay(lastIso) : '—'
    };
  }

  function getMonthlyTodoWhatsAppAuditButtonHtml() {
    if (!isMainAppSessionGenelYonetici()) return '';
    return '<button type="button" class="monthly-todo-wa-audit-btn" data-action="open-monthly-todo-wa-audit" title="WhatsApp Geçmişi" aria-label="WhatsApp Geçmişi">' +
      ...
      '<span class="monthly-todo-wa-audit-btn-label">WhatsApp Geçmişi</span>' +
      '</button>';
  }

  function syncMonthlyTodoWhatsAppAuditHeaderButton(modalEl) {
    if (!modalEl) return;
    var header = modalEl.querySelector('.modal-header');
    if (!header) return;
    var existing = header.querySelector('.monthly-todo-wa-audit-btn');
    if (!isMainAppSessionGenelYonetici()) {
      if (existing) existing.remove();
      return;
    }
    if (existing) return;
    var closeBtn = header.querySelector('.monthly-todo-modal-close');
    var html = getMonthlyTodoWhatsAppAuditButtonHtml();
    if (!html) return;
    if (closeBtn) closeBtn.insertAdjacentHTML('beforebegin', html);
    else header.insertAdjacentHTML('beforeend', html);
  }

  function buildMonthlyTodoWaAuditMaps() {
    var vehicleMap = Object.create(null);
    var vehicles = typeof window.getMedisaVehicles === 'function' ? (window.getMedisaVehicles() || []) : (window.appData && window.appData.tasitlar) || [];
    (Array.isArray(vehicles) ? vehicles : []).forEach(function(v) {
      if (v && v.id != null) vehicleMap[String(v.id)] = v;
    });
    ...
    return { vehicleMap: vehicleMap, branchNameMap: branchNameMap, branches: branches };
  }

  function getMonthlyTodoWaAuditTypeOptions(entries) {
    var seen = Object.create(null);
    var opts = [];
    (entries || []).forEach(function(row) {
      var code = String(row.typeCode || '').trim() || row.typeLabel;
      if (!code || seen[code]) return;
      seen[code] = true;
      opts.push({ value: String(row.typeCode || row.typeLabel), label: row.typeLabel || code });
    });
    opts.sort(function(a, b) { return String(a.label).localeCompare(String(b.label), 'tr'); });
    return opts;
  }

  function ensureMonthlyTodoWhatsAppAuditModalMounted() {
    var el = document.getElementById('monthly-todo-whatsapp-audit-modal');
    if (!el) {
      el = document.createElement('div');
      el.id = 'monthly-todo-whatsapp-audit-modal';
      el.className = 'modal-overlay monthly-todo-wa-audit-overlay';
      el.innerHTML =
        '<div class="modal-container monthly-todo-wa-audit-container">' +
          '<div class="modal-header">' +
            '<button type="button" class="modal-home" data-action="monthly-todo-wa-audit-back" aria-label="Aylık yapılacaklara dön" title="Geri">' +
            ...
            '</button>' +
            '<h2 class="premium-title">WHATSAPP BİLDİRİM GEÇMİŞİ</h2>' +
            '<button type="button" class="modal-close" data-action="monthly-todo-wa-audit-close" aria-label="Kapat">' +
              '<svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><line x1="18" y1="6" x2="6" y2="18"></line><line x1="6" y1="6" x2="18" y2="18"></line></svg>' +
            '</button>' +
          '</div>' +
          '<div class="modal-body monthly-todo-wa-audit-body"></div>' +
        '</div>';
      document.body.appendChild(el);
      wireMonthlyTodoWhatsAppAuditModalOnce(el);
    }
    return el;
  }

  function closeMonthlyTodoWhatsAppAuditModal(immediate) {
    var modal = document.getElementById('monthly-todo-whatsapp-audit-modal');
    if (!modal) return;
  ...
  function openMonthlyTodoWhatsAppAudit() {
    if (!isMainAppSessionGenelYonetici()) return;
    var modal = ensureMonthlyTodoWhatsAppAuditModalMounted();
    monthlyTodoWaAuditState.limit = MONTHLY_TODO_WA_AUDIT_PAGE_SIZE;
  ...
  window.openMonthlyTodoWhatsAppAudit = openMonthlyTodoWhatsAppAudit;

  function renderMonthlyTodoWhatsAppAuditModalContent() {
    var modal = document.getElementById('monthly-todo-whatsapp-audit-modal');
    var body = modal ? modal.querySelector('.monthly-todo-wa-audit-body') : null;
    if (!body) return;
    if (!isMainAppSessionGenelYonetici()) {
      body.innerHTML = '<div class="monthly-todo-wa-audit-empty">Bu geçmiş yalnız genel yönetici tarafından görüntülenebilir.</div>';
      return;
    }
    var maps = buildMonthlyTodoWaAuditMaps();
    var allEntries = normalizeMonthlyTodoWhatsAppAuditEntries(ensureMonthlyTodoWhatsAppLogs(), maps.vehicleMap, maps.branchNameMap);
    var filtered = filterMonthlyTodoWhatsAppAuditEntries(allEntries, monthlyTodoWaAuditState);
    var summary = summarizeMonthlyTodoWhatsAppAuditEntries(filtered);
    var typeOpts = getMonthlyTodoWaAuditTypeOptions(allEntries);
    var limit = Math.max(MONTHLY_TODO_WA_AUDIT_PAGE_SIZE, Number(monthlyTodoWaAuditState.limit) || MONTHLY_TODO_WA_AUDIT_PAGE_SIZE);
    var visible = filtered.slice(0, limit);
    var html = '';
    html += '<p class="monthly-todo-wa-audit-disclosure">Bu kayıtlar WhatsApp bağlantısının uygulama üzerinden başlatıldığını gösterir. Mesajın gönderildiği, teslim edildiği veya okunduğu anlamına gelmez.</p>';
    html += '<div class="monthly-todo-wa-audit-summary">';
    html += '<div class="monthly-todo-wa-audit-summary-item"><span class="monthly-todo-wa-audit-summary-label">Benzersiz bildirim kaydı</span><span class="monthly-todo-wa-audit-summary-value">' + escapeHtml(String(summary.uniqueCount)) + '</span></div>';
    html += '<div class="monthly-todo-wa-audit-summary-item"><span class="monthly-todo-wa-audit-summary-label">Toplam başlatma sayısı</span><span class="monthly-todo-wa-audit-summary-value">' + escapeHtml(String(summary.totalStarts)) + '</span></div>';
    html += '<div class="monthly-todo-wa-audit-summary-item"><span class="monthly-todo-wa-audit-summary-label">Son başlatma zamanı</span><span class="monthly-todo-wa-audit-summary-value">' + escapeHtml(summary.lastOpenedDisplay) + '</span></div>';
    html += '</div>';
    html += '<div class="monthly-todo-wa-audit-filters">';
    html += '<input type="search" class="form-input monthly-todo-wa-audit-search" data-audit-filter="search" placeholder="Plaka, kullanıcı, tür, şube ara..." value="' + escapeAttr(monthlyTodoWaAuditState.search || '') + '" aria-label="WhatsApp geçmişi arama">';
    html += '<select class="form-input monthly-todo-wa-audit-select" data-audit-filter="branch" aria-label="Şube filtresi">';
    html += '<option value="all"' + (monthlyTodoWaAuditState.branchId === 'all' ? ' selected' : '') + '>Tüm Şubeler</option>';
    (maps.branches || []).forEach(function(b) {
      if (!b || b.id == null) return;
      var id = String(b.id);
      html += '<option value="' + escapeAttr(id) + '"' + (String(monthlyTodoWaAuditState.branchId) === id ? ' selected' : '') + '>' + escapeHtml(String(b.name || '').trim() || 'İsimsiz Şube') + '</option>';
    });
    html += '</select>';
    html += '<select class="form-input monthly-todo-wa-audit-select" data-audit-filter="type" aria-label="Bildirim türü filtresi">';
    html += '<option value="all"' + (monthlyTodoWaAuditState.typeCode === 'all' ? ' selected' : '') + '>Tüm Türler</option>';
    typeOpts.forEach(function(opt) {
      html += '<option value="' + escapeAttr(opt.value) + '"' + (String(monthlyTodoWaAuditState.typeCode) === String(opt.value) ? ' selected' : '') + '>' + escapeHtml(opt.label) + '</option>';
    });
    html += '</select>';
    html += '<select class="form-input monthly-todo-wa-audit-select" data-audit-filter="time" aria-label="Zaman filtresi">';
    [
      { v: 'all', l: 'Tümü' },
      { v: '7d', l: 'Son 7 Gün' },
      { v: '30d', l: 'Son 30 Gün' },
      ...
      html += '<option value="' + opt.v + '"' + (monthlyTodoWaAuditState.timeFilter === opt.v ? ' selected' : '') + '>' + opt.l + '</option>';
    });
    html += '</select></div>';

    if (!allEntries.length) {
      html += '<div class="monthly-todo-wa-audit-empty">Henüz WhatsApp bildirim geçmişi bulunmuyor.</div>';
      body.innerHTML = html;
      return;
    }
    if (!filtered.length) {
      html += '<div class="monthly-todo-wa-audit-empty">Filtrelere uygun kayıt bulunamadı.</div>';
      body.innerHTML = html;
      return;
    }

    html += '<div class="monthly-todo-wa-audit-list" role="list">';
    visible.forEach(function(row) {
      var dateShown = row.date ? formatDateForDisplay(row.date) : '—';
      var firstShown = row.firstOpenedAt ? formatDateForDisplay(row.firstOpenedAt) : '—';
      var lastShown = row.lastOpenedAt ? formatDateForDisplay(row.lastOpenedAt) : '—';
      var branchTitle = row.branchIsCurrent ? 'Güncel şube' : 'Kayıt anındaki şube bilinmiyor; güncel eşleşme yok';
      html += '<article class="monthly-todo-wa-audit-card" role="listitem">';
      html += '<div class="monthly-todo-wa-audit-card-main">';
      html += '<div class="monthly-todo-wa-audit-plate">' + escapeHtml(row.plate) + '</div>';
      if (row.brandModel) html += '<div class="monthly-todo-wa-audit-meta">' + escapeHtml(formatBrandModel(row.brandModel)) + '</div>';
      html += '<div class="monthly-todo-wa-audit-meta" title="' + escapeAttr(branchTitle) + '">Şube: ' + escapeHtml(row.branchName) + '</div>';
      html += '<div class="monthly-todo-wa-audit-meta">Tür: ' + escapeHtml(row.typeLabel) + '</div>';
      html += '<div class="monthly-todo-wa-audit-meta">İlgili tarih: ' + escapeHtml(dateShown) + '</div>';
      html += '</div>';
      html += '<div class="monthly-todo-wa-audit-card-side">';
      html += '<div class="monthly-todo-wa-audit-meta">İlk: ' + escapeHtml(firstShown) + '</div>';
      html += '<div class="monthly-todo-wa-audit-meta">Son: ' + escapeHtml(lastShown) + '</div>';
      html += '<div class="monthly-todo-wa-audit-count">Başlatma: ' + escapeHtml(String(row.openedCount)) + '</div>';
      html += '<div class="monthly-todo-wa-audit-meta">Son başlatan: ' + escapeHtml(row.openedBy) + '</div>';
      html += '</div></article>';
    });
    html += '</div>';
    if (filtered.length > visible.length) {
      html += '<button type="button" class="monthly-todo-wa-audit-more" data-action="monthly-todo-wa-audit-more">Daha Fazla Göster</button>';
    }
    body.innerHTML = html;
  }

  function wireMonthlyTodoWhatsAppAuditModalOnce(modalEl) {
    if (!modalEl || modalEl._medisaWaAuditBound) return;
    modalEl._medisaWaAuditBound = true;
    modalEl.addEventListener('click', function(ev) {
      var t = ev.target;
      if (!t || typeof t.closest !== 'function') return;
      if (t.closest('[data-action="monthly-todo-wa-audit-close"]')) {
        ev.preventDefault();
        closeMonthlyTodoWhatsAppAuditModal();
        return;
      }
      if (t.closest('[data-action="monthly-todo-wa-audit-back"]')) {
        ev.preventDefault();
        closeMonthlyTodoWhatsAppAuditModal(true);
        openMonthlyTodoModal();
        return;
      }
      if (t.closest('[data-action="monthly-todo-wa-audit-more"]')) {
        ev.preventDefault();
        monthlyTodoWaAuditState.limit += MONTHLY_TODO_WA_AUDIT_PAGE_SIZE;
        renderMonthlyTodoWhatsAppAuditModalContent();
        return;
    ...
    modalEl.addEventListener('input', function(ev) {
      var el = ev.target;
      if (!el || el.getAttribute('data-audit-filter') !== 'search') return;
      monthlyTodoWaAuditState.search = el.value || '';
      monthlyTodoWaAuditState.limit = MONTHLY_TODO_WA_AUDIT_PAGE_SIZE;
      renderMonthlyTodoWhatsAppAuditModalContent();
    });
    modalEl.addEventListener('change', function(ev) {
      ...
      var kind = el.getAttribute('data-audit-filter');
      if (!kind || kind === 'search') return;
      if (kind === 'branch') monthlyTodoWaAuditState.branchId = el.value || 'all';
      if (kind === 'type') monthlyTodoWaAuditState.typeCode = el.value || 'all';
      if (kind === 'time') monthlyTodoWaAuditState.timeFilter = el.value || 'all';
      monthlyTodoWaAuditState.limit = MONTHLY_TODO_WA_AUDIT_PAGE_SIZE;
      renderMonthlyTodoWhatsAppAuditModalContent();
    });
  }

  window.addEventListener('medisa:monthly-todo-whatsapp-logs-changed', function() {
    var modal = document.getElementById('monthly-todo-whatsapp-audit-modal');
    if (modal && modal.classList.contains('active')) renderMonthlyTodoWhatsAppAuditModalContent();
  });

  // Expose pure helpers for isolated verifier (no parallel store).
  window.__medisaMonthlyTodoWaAuditTestApi = {
  ...
  function getMonthlyTodoBranchFilterHtml(branches) {
    ...
    var isFiltered = !!selectedBranch;
    var selectedLabel = isFiltered
      ? (String(selectedBranch.name || '').trim() || 'İsimsiz Şube')
      : 'Tüm Şubeler';
  ...
  function bindMonthlyTodoModalDelegatedInteraction(modalEl) {
    ...
    function onMonthlyTodoModalClick(ev) {
      ...
      var auditBtn = target.closest('[data-action="open-monthly-todo-wa-audit"]');
      if (auditBtn && modalEl.contains(auditBtn)) {
        ev.preventDefault();
        ev.stopPropagation();
        openMonthlyTodoWhatsAppAudit();
        return;
      }
      if (target.closest('.monthly-todo-wa-link')) return;
      var row = target.closest('.monthly-todo-task-row');
  ...
  function syncMonthlyTodoModalHeader(modalEl) {
    ...
    syncMonthlyTodoWhatsAppAuditHeaderButton(modalEl);
  }

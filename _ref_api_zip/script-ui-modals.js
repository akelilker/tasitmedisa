/* ====================================================================
   ...
   (GÜNCELLEME v35: Manuel Borç/Alacak Bildirimleri Eklendi)
   ==================================================================== */
...
function closeModal(id) {
    ...
    if (modal) {
        ...
        if (id === 'category') { }
        if (id === 'tasitlar') {
             closeVehicleDetailView();
        }

        // --- MOBİL SCROLL FIX (GERİ YÜKLEME) ---
        setTimeout(() => {
...
function renderNotifications() {
             ...
             if (daysDiff <= DEBT_ALERT_DAYS) {
                ...
                notificationsHtml += `
                    <div class="cari-item ${statusClass}" onclick="openModal('fin'); showFinanceDetail('payables'); closeNotificationsDropdown();">
                        <div>
                            <span class="cari-item-name">${person ? person.name : 'Bilinmiyor'} (Planlı Borç)</span>
    ...
    // 2. Manuel Borç/Alacak Vadeleri (YENİ EKLENEN)
    const datedTxs = data.transactions.filter(t => 
        t.status === 'aktif' && 
        t.due_date && 
        (t.category === 'alinan-borc' || t.category === 'verilen-borc')
    );

    datedTxs.forEach(tx => {
        const currentBalance = calculateCariBalance(tx.person_id);
        const isRelevant = (tx.category === 'alinan-borc' && currentBalance < -0.01) || 
                           (tx.category === 'verilen-borc' && currentBalance > 0.01);
        ...
        if (isRelevant) {
             ...
             if (daysDiff <= DEBT_ALERT_DAYS) {
                 // Aynı kişi ve tipte daha önce bildirim eklenmemişse ekle (tekrarları önlemek için)
                 // Not: Basitlik için şimdilik her işlemi ayrı gösteriyoruz, gerekirse gruplanabilir.
                 notificationCount++;
                 const person = data.persons.find(p => p.id === tx.person_id);
                 let statusClass = daysDiff < 0 ? 'danger' : 'warn';
                 let statusText = daysDiff < 0 ? `Vadesi ${-daysDiff} gün geçti` : (daysDiff === 0 ? 'Son gün bugün' : `Vadeye ${daysDiff} gün kaldı`);

                 notificationsHtml += `
                    <div class="cari-item ${statusClass}" onclick="openModal('fin'); showPersonDetail(${tx.person_id}, true); closeNotificationsDropdown();">
                        <div>
                            <span class="cari-item-name">${person ? person.name : 'Bilinmiyor'} (${typeText})</span>
                            <div style="font-size:12px; color: var(--muted); margin-top: 4px;">Vade Tarihi: ${fdate(tx.due_date)}</div>
                        </div>
                        <div style="text-align: right;">
                             <div class="cari-item-balance ${tx.category === 'alinan-borc' ? 'neg' : 'pos'}">${fmt(tx.amount)}</div>
                             <div style="font-size:11px; font-weight: bold; margin-top: 4px;">${statusText}</div>
                        </div>
                    </div>`;
             }

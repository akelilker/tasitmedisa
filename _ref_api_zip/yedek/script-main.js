/* ====================================================================
window.load=function(){console.log("Loading...")};
   DOSYA 3/3: script-main.js
   Açıklama: Form gönderme mantığı, olay dinleyiciler (event listeners)
   ve uygulamanın başlangıç (initialization) kodlarını içerir. (v29 - 6 Maddelik Geliştirmeye Göre Güncellendi)
   ==================================================================== */
...
async function handleTasitAlimi(txData, amount) {
    ...
    const newVehiclePlate = document.getElementById('new_vehicle_plate').value;
    ...
    if (purchaseMethod === 'pesin') {
        ...
        if (downPayment >= 0) data.transactions.push({ ...txData, amount: downPayment, vehicle_id: newVehicle.id });
        ...
        // MADDE 6: Ortaklık verisini 'vehicle' nesnesine kaydet
        newVehicle.partnerships = newVehicleOwnership; 
        data.vehicles.push(newVehicle);
        ...
        newVehicleOwnership.forEach(p => {
            if (p.type === 'company') {
                ...
                data.transactions.push({ id: nextId('tx'), date: txData.date, category: 'tasit-alimi', amount: p.amount, note: `${newVehicle.plate} alımı - Ortak Payı`, person_id: p.person_id, vehicle_id: newVehicle.id, status: 'aktif', account: 'not-applicable' }); // Ortak payı için hesap türü 'not-applicable'
            }
        });
    }
    return true;
...
async function handleTasitSatisi(txData, amount) {
    ...
    if(!v) { toast('Satılacak taşıt seçilmelidir.', 'err'); updateStatusIndicator('error', 'Eksik Bilgi'); submitBtn.disabled = false; return false; }
    ...
    v.status = 'satildi';
    v.sale_price = amount; // DİKKAT: Bu 'sale_price' Madde 5'teki hesabı etkileyecek
    ...
    if (saleType === 'pesin') {
        ...
        if (remaining > 0) {
            const installmentAmount = remaining / instCount;
            newSchedule = { id: nextId('schedule'), type: 'alacak', person_id: buyerPersonId, vehicle_id: v.id, total_amount: remaining, paid_amount: 0, remaining, status: 'devam-ediyor', installments: [] };
    ...
    const profit = v.sale_price - v.purchase_price - totalExpenses;
    data.transactions.push({id: nextId('tx'), date: txData.date, category: 'tasit-satis-kari', amount: profit, vehicle_id: v.id, status: 'aktif', virtual: true});
...
async function handleGelenTL(txData, amount) {
    ...
    const hasPending = data.schedules.some(s => s.person_id === personId && s.type === 'alacak' && s.status !== 'tamamlandi' && s.status !== 'iptal');
    if (hasPending && !scheduleId) {
        toast("Bu kişinin bekleyen alacağı var. Lütfen bir ödeme planı seçin.", "err");
...
/* ====================================================================
   ...
   ==================================================================== */
// GÜNCELLENDİ (v28.2.14 - Focus Hatası Nihai Çözümü)
async function submitTx(e) {
	e.preventDefault();
	clearFormErrors();

    // ====== FOKUSLANABİLİRLİK HATASI ÇÖZÜMÜ (v28.2.14) ======
    // ADIM 1: Kategori bazında required'ı yönet
    ...
    // Taşıt alımında amount gizli olduğu için required kaldır
    if (cat === 'tasit-alimi') {
    ...
    // ADIM 2: Tüm gizli alanların required'ını kaldır
    // (offsetParent === null = element görünür değil/gizli)
    ...
    // GÜNCELLEME (Madde 3): Tutar, kategoriye göre alınır
    let amount;
    if (cat === 'tasit-alimi') {
    ...
    // --- Bakiye Kontrolü (Sadece Giderler İçin) ---
    ...
    if (flowType === 'out') {
        ...
        if (cat === 'tasit-alimi') {
            ...
            if (purchaseMethod === 'vadeli') {
                ...
                const companyShareInput = document.getElementById('company_share_amount');
                if (companyShareInput) {
                    amountToCheck = parseN(companyShareInput.value);
        ...
        if (amountToCheck > 0 && currentBalance < amountToCheck) {
			if(errorInputEl) errorInputEl.classList.add('input-error');
            toast('Yetersiz bakiye! Bu işlem için ' + fmt(amountToCheck) + ' gerekli, ' + txData.account + ' hesabında ' + fmt(currentBalance) + ' var.', "err");
            updateStatusIndicator('error', 'Yetersiz Bakiye');
            submitBtn.disabled = false;
            return;
        }
    ...
    let success = false;
    // --- İŞLEM YÖNLENDİRMESİ VE MANTIĞI ---
	switch(cat) {
        ...
        case 'asli-ortak-sermaye-cikis':
            const person = data.persons.find(p => p.id === txData.person_id);
            if (!txData.person_id || person.type !== 'asli') {
                toast("Sermaye işlemi için Asli Ortak seçimi zorunludur.", "err");
                submitBtn.disabled = false;
                return;
            }
            data.transactions.push(txData);
    ...
    if (success) {
        ...
        // --- GÜNCELLEME (Madde 5 Entegrasyonu) ---
        // Kaydedilen işlemin vehicle_id'sini al
        const savedVehicleId = txData.vehicle_id || tempLinkedVehicleId;
        ...
        // Eğer bir araçla ilgiliyse VE o aracın detay kartı açıksa, kartı yenile
        if (savedVehicleId &&
            document.getElementById('vehicle-detail-view')?.style.display === 'block' &&
            editingVehicleDetailId === savedVehicleId) 
        {
            if (typeof showVehicleDetailViewAdvanced === 'function') {
                showVehicleDetailViewAdvanced(savedVehicleId);
            }
        }
        // ------------------------------------------
...
function editTx(id) {
    ...
    if (!tx) { toast("İşlem bulunamadı.", "err");
		return; }

    if (['tasit-alimi', 'tasit-satisi', 'kasa-banka-virman'].includes(tx.category)) {
        toast("Bu tür karmaşık işlemler (Alım/Satım/Virman) şu an düzenlenemez.", "warn");
        return;
    }
...
async function cancelTx(txId) {
    ...
    showConfirm("Bu işlemi iptal etmek istediğinizden emin misiniz? Bu işlem geri alınamaz.", async () => {
        ...
        if (tx.category === 'tasit-satisi' && tx.vehicle_id) {
            ...
            const profitTx = data.transactions.find(t => t.vehicle_id === v.id && t.category === 'tasit-satis-kari');
			if(profitTx) profitTx.status = 'iptal';
        ...
        // --- GÜNCELLEME (Madde 5 Entegrasyonu) ---
        // İptal edilen işlemin vehicle_id'sini al
...
function handleKonsinyeSave(e) {
    e.preventDefault();
    const plate = document.getElementById('konsinye-plate').value.trim();
...
document.addEventListener("DOMContentLoaded", () => {
	load().then(() => {
        ...
        // BURAYA EKLE: Migration fonksiyonunu çağır
        migrateVehicleDetailsFields();
    ...
    // MADDE 5 (Kar/Zarar) için formatlama
    ...
    applyBodyLock();
    document.addEventListener('click', function(){ setTimeout(applyBodyLock, 0); });
});
...
function returnKonsinyeVehicle(vehicleId) {
    if (typeof showConfirm === 'function') showConfirm("Bu konsinye aracı sahibine iade etmek istediğinizden emin misiniz? Araç stoktan kaldırılacaktır.", () => {
        const vehicle = data.vehicles.find(v => v.id === vehicleId);
        if(vehicle) {
            vehicle.status = 'iade-edildi';
            save();
            if (typeof checkAllNotifications === 'function') checkAllNotifications();
            if(document.getElementById('modal-tasitlar').classList.contains('on')) {
                 if (typeof renderTasitlarData === 'function') renderTasitlarData('konsinye');
            }
            toast("Araç sahibine iade edildi.", "info");
        }
    });
}

// --- SİLİNDİ: clearCacheAndReload() fonksiyonu buradan kaldırıldı. ---

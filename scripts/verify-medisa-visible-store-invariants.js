...
function runStaticTests() {
  ...
  test('20 UI modüllerinde görünür reader owner kontratı', function() {
    ...
    Object.keys(readerExpectations).forEach(function(file) {
      ...
      const violations = scanLegacyVisibleReads(source);
      assert.equal(violations.length, 0, file + ' legacy visible read: ' + JSON.stringify(violations.slice(0, 3)));
    });
  });
  ...
  test('23 data-service doğrudan taşıt assignment yapmıyor', function() {
    const hits = scanDirectAssignments(dataServiceSource, 'data-service.js');
  ...
  test('31 K2 toplu sync clone/write-once owner', function() {
    ...
    assert.match(fn, /getMedisaVehicles/);
    assert.match(fn, /Object\.assign\(\{\},\s*vehicle/);
    assert.match(fn, /syncedCount === 0/);
    assert.match(fn, /writeVehicles/);
  });
  ...
  test('32 Kasko toplu sync clone/write-once owner', function() {
    ...
    assert.match(fn, /getMedisaVehicles/);
    assert.match(fn, /Object\.assign\(\{\},\s*v/);
    assert.match(fn, /if \(!changed\) return false/);
    assert.match(fn, /writeVehicles/);
  });

  test('33 no-op K2 writeVehicles çağrısı guard', function() {
    const fn = extractFunctionSource(read('ayarlar.js'), 'syncActiveVehicleTasitKartiExpiryWithK2');
    assert.match(fn, /if \(syncedCount === 0\) return 0/);
    assert.doesNotMatch(fn, /writeVehicles[\s\S]{0,40}syncedCount === 0/);
  });

  test('34 no-op kasko writeVehicles çağrısı guard', function() {
    const fn = extractFunctionSource(dataServiceSource, 'guncelleTumKaskoDegerleri');
    const writeIdx = fn.indexOf('writeVehicles');
    const guardIdx = fn.indexOf('if (!changed) return false');
  ...
  test('39 save-wire kontratı korunuyor', function() {
    assert.match(dataManagerSource, /function buildSaveWirePayload\(/);
    assert.match(dataManagerSource, /window\.buildSaveWirePayload\s*=\s*buildSaveWirePayload/);
    assert.match(dataServiceSource, /Compatibility wrapper — canonical owner window\.writeVehicles/);
    assert.doesNotMatch(dataServiceSource, /window\.appData\.tasitlar\s*=/);
  });
}

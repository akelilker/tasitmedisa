...
test('Şifre talebi endpointi ve yenilenen driver assetleri deploy zincirindedir', () => {
  ...
  assert.match(files.driverScript, /bootstrap:\s*'20260926\.1'/);
  assert.match(files.driverScript, /login:\s*'20260921\.1'/);
  assert.match(files.driverScript, /shellCss:\s*'20260925\.6'/);
  assert.match(files.loginHtml, /driver-shell\.css\?v=20260925\.6/);
  assert.match(files.loginHtml, /driver-script\.js\?v=20260926\.1/);
});

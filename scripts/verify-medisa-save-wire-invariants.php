<?php
/**
 * PERF-P0-3 — save wire delta-v1 / legacy-full invariantleri.
...
$userContext = ['role' => 'kullanici', 'user_id' => 'u-user', 'branch_ids' => []];
$userDenied = (($userContext['role'] ?? 'kullanici') === 'kullanici')
    ? medisaBuildErrorResult('Bu ekran için yetkiniz yok.', 403)
    : ['success' => true];
swAssert('Kullanici role 403 gate', (int)($userDenied['status'] ?? 0) === 403);

// --- 1) Legacy full payload ---
$data = swFixtureBase();
$legacyIncoming = $data;
...
$deltaUpdate = swDeltaWire([
    ...
    'changedVehicleIds' => ['v1'],
    'deletedVehicleIds' => [],
    'deletedVehicleVersions' => (object)[],
], [
    'tasitlar' => [['id' => 'v1', 'plate' => '34 AAA 1', 'version' => 3, 'branchId' => 'b1', 'km' => '999']],
]);
$deltaUpdate['_medisaMutation']['deletedVehicleVersions'] = [];
...
// --- 9-12) Delta vehicle validation 400 ---
$data = swFixtureBase();
...
// --- 13-18) Collection / wire validation ---
$data = swFixtureBase();
...
// --- 25) Users credential preservation ---
$data = swFixtureBase();
...
// --- Password changes still rejected ---
$data = swFixtureBase();
...
swAssert('data-manager no full Object.assign appData save', strpos($dm, 'Object.assign({}, window.appData)') === false || strpos($dm, 'buildSaveWirePayload') !== false);
swAssert('data-manager excludes kaskoDegerListesi', strpos($dm, 'delete payloadObj.kaskoDegerListesi') !== false || strpos($dm, 'delete wirePayload.kaskoDegerListesi') !== false);

// Allowlist does not silently persist driver collections via save

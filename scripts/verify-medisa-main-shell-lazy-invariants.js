...
if (implementationPresent) {
  ...
  test('ilk giriş parola önerisi core shell içinde kalır', function() {
    assert.match(index, /id="main-password-modal"/);
    assert.match(index, /id="main-password-suggestion"/);
  });
  ...
  test('version ve SW cache beklenen değerde', function() {
    assert.match(core, /tasitlar: '20260717\.4'/);
    assert.match(index, /script-core\.js\?v=20260717\.4/);
    assert.match(sw, /medisa-v2\.242/);
  });

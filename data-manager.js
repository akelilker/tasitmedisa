...
function replaceMedisaCollection(kind, nextList, options) {
    var appKey = MEDISA_COLLECTION_APP_KEYS[kind];
    if (!appKey) return getRawMedisaCollection(kind).slice();
    if (!window.appData) window.appData = getDefaultAppData();
...
function commitMedisaAppDataSnapshot(nextAppData, options) {
    ...
    window.appData = {
        ...
        ayarlar: incoming.ayarlar && typeof incoming.ayarlar === 'object' ? incoming.ayarlar : (current.ayarlar || getDefaultAppData().ayarlar),
        ...
        kaskoDegerListesi: incoming.kaskoDegerListesi && typeof incoming.kaskoDegerListesi === 'object'
            ...
            : (current.kaskoDegerListesi || { updatedAt: '', period: '', sourceFileName: '', rows: [] }),
...
function setMedisaSession(sessionData) {
    ...
    clearMedisaVisibleCacheSlot('branches');
    applyMainAppSessionUiState();
    queueMedisaCollectionsChanged(['vehicles', 'branches', 'users'], 'session-change');
}
...
function patchMainAppSessionUserRecord(patch) {
    ...
    if (!userId) return;
    var users = getRawMedisaCollection('users').slice();
    var found = false;
    for (var i = 0; i < users.length; i++) {
        if (String(users[i].id || '') !== userId) continue;
        users[i] = Object.assign({}, users[i], patch);
        found = true;
        break;
    }
    if (!found) return;
    replaceMedisaCollection('users', users, { reason: 'session-user-patch' });
}

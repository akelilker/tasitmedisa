...
- `EVENT_MENU_EVENT_ICONS` — 16 doğrudan olay ikonu + fallback `vehicle` (Sigorta/Kasko bu registry'de ayrı key taşımaz)

`getEventMenuCategoryIconHtml()` bu registry'lerden HTML üretir. Sigorta ve Kasko olay kartları, özel tikli kalkan + S/K harfi üretim akışını kullanır (`categoryId === 'sigorta'|'kasko'`).
...
| `index.html` → `script-core.js` query | `20260611.18` |
| `sw.js` → `CACHE_VERSION` | `medisa-v2.151` |
...
| Faz 3D | Kapanış raporu düzeltmesi ve statik doğrulama | Tamam |
...
### Belge / Ruhsat bloğu — kapsam dışı (Olay Ekle fazları)
...
**İstisna (2026-06):** Sigorta/Kasko/Takograf belge upload tarih bağlantısı uygulandı — ayrı rapor: [`belge-yukleme-tarih-baglantisi-gelistirici-raporu.md`](belge-yukleme-tarih-baglantisi-gelistirici-raporu.md).
...
## 8. Kapanış Doğrulama Durumu

**Başlangıç kapısı (2026-06-11, commit `93d2cb5d`):** branch `main`, HEAD = origin/main, working tree temiz.

### 8.1 Statik kod kontrolleri — GEÇTİ

| Kontrol | Sonuç |
| --- | --- |
| `EVENT_DEFINITIONS` kayıt sayısı | 18 |
| `EVENT_MENU_EVENT_ICONS` | 16 doğrudan ikon + `vehicle` fallback |
| Sigorta/Kasko özel S/K ikon akışı | Mevcut (`getEventMenuCategoryIconHtml`) |
| `.event-form-stack` form kökü | 18 adet |
| `EVENT_TITLES` | Bulunmuyor |
| Local `saveHandlers` map | Bulunmuyor |
| Handler çözümleme | `window[eventDefinition.saveHandlerName]` |
| `.detail-plate-row` fallback oluşturma | Mevcut (Faz 4 backlog) |
| Marka satırı migration kodu | Mevcut (Faz 4 backlog) |
| Olay Ekle buton remove/recreate akışı | Mevcut (Faz 4 backlog) |
| `.history-btn-minimal` CSS | `flex-shrink: 0` yanında renk, hover, display, reset kuralları taşır; Faz 4'te analizsiz silinmemeli |

**Sözdizimi:** `node --check tasitlar.js`, `node --check script-core.js`, `node --check sw.js` ve `php -l core.php` hatasız.
...
- Runtime smoke veya yeni arayüz değişiklikleri sonrasında ortaya çıkabilecek ek legacy selector adaylarının yeniden taranması.

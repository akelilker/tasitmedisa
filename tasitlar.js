  ...
  window.updateNotifications = function() {
    ...
    if (true) {
      ...
      if (!localStorage.getItem(mtvKey)) {
        ...
        mtvHtml = '<div class="notification-item date-warning-orange-border mtv-notification" style="display: flex; justify-content: space-between; align-items: center; padding: 10px 12px;"><div style="flex: 1; text-align: left; padding-right: 4px; line-height: 1.3;"><strong style="color: var(--orange-warn); font-size: 13px;">MTV Hatırlatması</strong><br><span style="font-size: 11.5px; color: var(--txt-muted); white-space: normal; display: block;">Ayın Son Gününe Kadar MTV Ödemelerinin Yapılması Gerekmektedir.</span></div><div class="mtv-dismiss-wrapper"><button type="button" class="mtv-dismiss-btn" onclick="dismissMTVNotif(event, \'' + mtvKeyEsc + '\')" aria-label="Bildirimi Kapat"><svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><polyline points="20 6 9 17 4 12"></polyline></svg></button><div class="mtv-tooltip">Ödeme Yapıldıysa Bildirimi Silebilirsiniz.</div></div></div>';
        hasOrange = true;
      }
    }

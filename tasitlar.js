...
(function() {
  const MEDISA_TASITLAR_MODULE_VERSION = '20260926.12';
  ...
  var EVENT_MENU_EVENT_ICONS = {
    ...
    sigorta: '<svg viewBox="0 0 24 24" aria-hidden="true"><path d="M12 22s8-4 8-10V5l-8-3-8 3v7c0 6 8 10 8 10z"/><path d="M7.6 11.3 10.3 14 16.4 7.9"/><text x="12" y="18.1" style="fill:currentColor;stroke:none;font-family:Arial,sans-serif;font-size:3.8px;font-weight:800;text-anchor:middle">S</text></svg>',
    kasko: '<svg viewBox="0 0 24 24" aria-hidden="true"><path d="M12 22s8-4 8-10V5l-8-3-8 3v7c0 6 8 10 8 10z"/><path d="M7.6 11.3 10.3 14 16.4 7.9"/><text x="12" y="18.1" style="fill:currentColor;stroke:none;font-family:Arial,sans-serif;font-size:3.8px;font-weight:800;text-anchor:middle">K</text></svg>',
  ...
  window.showVehicleDetail = function(vehicleId) {
    ...
    if (detailToolbar) {
      ...
      let backLabel = returnToMonthlyTodoAfterVehicleDetail ? 'Bu Ay Yap\u0131lacaklar' : 'Taşıtlar';
  ...
  function backFromVehicleDetailToListContext() {
    if (returnToMonthlyTodoAfterVehicleDetail) {
      returnToMonthlyTodoAfterVehicleDetail = false;
      window.dispatchEvent(new CustomEvent('medisa:open-monthly-todo-return'));
      return;
    }
  ...
  function getVehicleDocumentIconSvg(cfg) {
    if (!cfg) return '';
    ...
    if (cfg.key === 'sigorta') {
      return svgOpen +
        ...
        '<path d="M7.6 11.3 10.3 14 16.4 7.9"></path>' +
        '<text x="12" y="18.1" fill="currentColor" stroke="none" font-family="Arial, sans-serif" font-size="3.8" font-weight="800" text-anchor="middle">S</text>' +
        svgClose;
    }
    if (cfg.key === 'kasko') {
      return svgOpen +
  ...
  /** Kısa ömürlü belge doc-token önbelleği (vehicleId::documentType veya settings::dtype). */
  var medisaDocTokenCache = new Map();
  var MEDISA_DOC_TOKEN_CACHE_MARGIN_MS = 30000;

  function getMedisaDocumentTokenCacheKey(vehicleId, documentType) {
  ...
  function parseMedisaDocumentUrlContext(rawUrl) {
    try {
      ...
      return {
        vehicleId: String(u.searchParams.get('id') || '').trim(),
        documentType: String(u.searchParams.get('documentType') || 'ruhsat').trim() || 'ruhsat'
      };
    } catch (e) {
      return { vehicleId: '', documentType: 'ruhsat' };
    }
  }

  function appendDocTokenToDocumentUrl(rawUrl, docToken) {
    ...
    try {
      ...
      if (docToken) {
        u.searchParams.set('doc', docToken);
      } else {
  ...
  function mintMedisaDocumentToken(vehicleId, documentType) {
    const dt = String(documentType || 'ruhsat').trim() || 'ruhsat';
    const cfg = getVehicleDocumentConfig(dt);
    const cacheKey = getMedisaDocumentTokenCacheKey(vehicleId, dt);
    const now = Date.now();
    if (cacheKey) {
      const cached = medisaDocTokenCache.get(cacheKey);
      if (cached && cached.token && cached.expiresAtMs > now + MEDISA_DOC_TOKEN_CACHE_MARGIN_MS) {
        return Promise.resolve(cached);
      }
    }

    const body = { documentType: dt };
    if (cfg.scope !== 'settings') {
      const vid = String(vehicleId || '').trim();
      if (!vid) {
        return Promise.reject(new Error('document-id-missing'));
      }
      body.vehicleId = vid;
    }
      ...
      .then(function(response) {
        return response.json().then(function(data) {
          if (!response.ok || !data || data.ok !== true || !data.token) {
            const err = new Error('document-token-mint-failed');
          ...
          if (cacheKey) {
            medisaDocTokenCache.set(cacheKey, entry);
          }
          return entry;
        });
      });
  ...
  function resolveMedisaDocumentAccessUrl(rawUrl, vehicleId, documentType) {
    const base = String(rawUrl || '').trim();
    if (!base) {
      return Promise.reject(new Error('document-url-missing'));
    }
    if (!getMedisaPortalToken()) {
      return Promise.resolve(toAbsoluteRuhsatUrl(base));
    }

    const parsed = parseMedisaDocumentUrlContext(base);
    const dt = String(documentType || parsed.documentType || 'ruhsat').trim() || 'ruhsat';
    const cfg = getVehicleDocumentConfig(dt);
    const vid = cfg.scope === 'settings'
      ? ''
      : String(vehicleId || parsed.vehicleId || window.currentDetailVehicleId || '').trim();

    return mintMedisaDocumentToken(vid, dt).then(function(entry) {
      return appendDocTokenToDocumentUrl(base, entry.token);
    });
  }
  ...
  /**
   * Belgeyi yeni sekmede açar. Oturum varsa doc-token ile ruhsat.php açılır (doğru sekme/dosya adı).
  ...
  function openVehicleDocumentInNewTab(vehicleId, documentUrl, documentType, pdfViewerFragment) {
    ...
    if (getMedisaPortalToken()) {
      ...
      return resolveMedisaDocumentAccessUrl(docUrl, vid, dt)
  ...
  function wireIosPwaPdfStagingIframe(cacheKey, pageUrls) {
    ...
    try {
      frame.srcdoc = buildIosPwaPdfPrintHtml(pageUrls);
    } catch (wirePdfSrcErr) {
      detachIosPwaPdfStagingFrame(entry);
    }
  }
  ...
  /**
   * Canonical document viewer (desktop / mobil / PWA ortak owner): ruhsat.php Blob +
   * PDF.js / <img> + Kaydet-Paylaş + ayrı Yazdır. Imagick sunucu önizlemesine bağımlılık yok.
  ...
  function openMedisaCanonicalDocumentViewer(vehicleId, documentType, viewerOpts) {
    ...
    var hostIsIos = isIosCanonicalDocumentViewerHost();
    var zoomLevel = 1;
    var MEDISA_VIEWER_MIN_ZOOM = 0.5;
    var MEDISA_VIEWER_MAX_ZOOM = 3;
    var MEDISA_VIEWER_ZOOM_STEP = 0.25;
    ...
    function updateZoomButtons() {
      if (!shellApi || hostIsIos) return;
      shellApi.setActionState('zoom-out', { disabled: zoomLevel <= MEDISA_VIEWER_MIN_ZOOM });
      shellApi.setActionState('zoom-in', { disabled: zoomLevel >= MEDISA_VIEWER_MAX_ZOOM });
    }

    /** PDF.js sayfalarını zoom seviyesine göre yeniden ölçekle (page host genişliği owner'ı). */
    function applyZoomLevel() {
      if (!pdfDoc || !scrollEl || isStale()) return;
      Object.keys(pageState).forEach(function(pageKey) {
        releasePageCanvas(Number(pageKey));
      });
      pageState = Object.create(null);
      var rootRect = scrollEl.getBoundingClientRect();
      scrollEl.querySelectorAll('.medisa-doc-viewer-page').forEach(function(host) {
        host.style.width = (zoomLevel * 100) + '%';
        var rect = host.getBoundingClientRect();
        if (rect.bottom >= rootRect.top - 120 && rect.top <= rootRect.bottom + 120) {
          renderVisiblePdfPage(Number(host.dataset.page), host).catch(function() {});
        }
      });
      updateZoomButtons();
    }
    ...
    function changeZoom(delta) {
      var next = Math.min(MEDISA_VIEWER_MAX_ZOOM, Math.max(MEDISA_VIEWER_MIN_ZOOM, zoomLevel + delta));
    ...
    function runPrintAction() {
      ...
      if (!hostIsIos) {
        // Desktop/masaüstü: mevcut authenticated orijinal belge + native yazdırma owner'ı korunur.
        openVehicleDocumentInNewTab(
          vid,
          documentUrl,
          dt,
          cachedEntry && cachedEntry.kind === 'image' ? '' : 'toolbar=1&navpanes=0&zoom=page-width&view=FitH'
        ).catch(function(printOpenErr) {
          console.error(cfg.label + ' yazdırma penceresi açılamadı', printOpenErr);
        });
        return;
      }
    ...
    function buildViewerActions() {
      var actions = [];
      if (!hostIsIos) {
        actions.push({ id: 'zoom-out', label: '−' });
        actions.push({ id: 'zoom-in', label: '+' });
      }
      actions.push({ id: 'share', label: 'Hazırlanıyor…', disabled: true });
    ...
    shellApi = window.openMedisaPreviewShell({
      ...
      onAction: function(actionId) {
        if (actionId === 'zoom-in') {
          changeZoom(MEDISA_VIEWER_ZOOM_STEP);
          return;
        }
  ...
  window.openMedisaCanonicalDocumentViewer = openMedisaCanonicalDocumentViewer;

  /** iOS geriye dönük ad: canonical viewer sarmalayıcısı (mevcut iOS çağrı yüzeyi korunur). */
  function openIosCanonicalDocumentViewer(vehicleId, documentType, viewerOpts) {
    return openMedisaCanonicalDocumentViewer(vehicleId, documentType, viewerOpts);
  }
  ...
  function openRuhsatPrintDialog(ruhsatUrl, vehicleId, documentType, opts) {
    ...
    function doPrint() {
      try {
        ...
        if (fallbackOpenUrl) {
          if (getMedisaPortalToken()) {
            resolveMedisaDocumentAccessUrl(documentUrl, vehicleId, dt)
              .then(function(authedFb) {
    ...
    if (viewerOptions.showPrintButton) {
      ...
      printBtn.onclick = function() {
        ...
        if (viewerOptions.forceExternalPrint) {
          if (getMedisaPortalToken()) {
            const printTab = openBlankDocumentTab();
            resolveMedisaDocumentAccessUrl(documentUrl, vehicleId, dt)
              .then(function(op) {
                const target = isImage
                  ? op
                  : buildPdfViewerUrl(op, 'toolbar=1&navpanes=0&zoom=page-width&view=FitH');
                if (printTab && !printTab.closed) {
                  try {
                    printTab.location.href = target;
                    printTab.focus();
                  } catch (e) {
                    openUrlInNewTab(target, printTab);
                  }
                  return;
                }
                openUrlInNewTab(target);
              ...
              .catch(function() {
                if (printTab && !printTab.closed) {
                  try { printTab.close(); } catch (closeErr) {}
                }
                if (fallbackUrl) {
        ...
        if (getMedisaPortalToken()) {
          const printTab2 = openBlankDocumentTab();
          resolveMedisaDocumentAccessUrl(documentUrl, vehicleId, dt)
            .then(function(op2) {
              const target = isImage
                ? op2
                : buildPdfViewerUrl(op2, 'toolbar=1&navpanes=0&zoom=page-width&view=FitH');
              if (printTab2 && !printTab2.closed) {
                try {
                  printTab2.location.href = target;
                  printTab2.focus();
                } catch (e) {
                  openUrlInNewTab(target, printTab2);
                }
                return;
              }
              openUrlInNewTab(target);
            ...
            .catch(function() {
              if (printTab2 && !printTab2.closed) {
                try { printTab2.close(); } catch (closeErr) {}
              }
              if (fallbackUrl) {
                openUrlInNewTab(fallbackUrl);
              } else if (typeof window.viewRuhsatPdf === 'function') {
        ...
        .catch(function(err) {
          fetchRuhsatDocumentObjectUrl(vehicleId, documentUrl, dt)
            ...
            .catch(function(fetchErr) {
              console.error(cfg.label + ' görüntüleme hazırlanamadı', fetchErr || err);
              var st = (fetchErr && fetchErr.httpStatus) || (err && err.httpStatus);
              if (st === 404) {
    ...
    if (hasDoc) {
      ...
      if (iosCanonical) {
        ...
        previewBtn.onclick = function(e) {
          ...
          e.stopPropagation();
          if (!ruhsatIsImage) {
            // PDF: canonical viewer (authenticated orijinal belge + PDF.js); Imagick preview yok.
            openMedisaCanonicalDocumentViewer(vid, dt).catch(function(err) {
              console.error(cfg.label + ' açılamadı', err);
            });
            return;
          }
  ...
  function renderHistoryDigerEventHtml(event, vehicle, branches) {
    ...
    const isPolicyExpiryHistoryEvent = eventType === 'kasko-guncelle' || eventType === 'sigorta-guncelle';
    const policyExpiryIso = isPolicyExpiryHistoryEvent
      ? getPolicyDateIso(eventData.bitisTarihi)
      : '';
    const policyExpiryDisplay = policyExpiryIso
      ? formatDateForDisplay(policyExpiryIso)
      : '';
    const dateText = escapeHtml((
      isPolicyExpiryHistoryEvent
        ? (policyExpiryDisplay || actionDateText)
        : (isDateRenewalEvent ? actionDateText : formatDateForDisplay(event.date))
    ) || '-');
